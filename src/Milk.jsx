...
function Milk() {
  let milk = useSelector(state => state.products.milk) || []; // Fallback to empty array
  ...
  const filteredItems = milk.filter((item) => {
    ...
    const itemPrice = item.price && typeof item.price === 'number' ? item.price.toString().toLowerCase() : '';

    return (
      itemName.includes(searchQueryLower) || // Match name
      itemPrice.includes(searchQueryLower)    // Match price
    );
  });
